...
1. Shell 传入菜单项相对编号。
2. 如果编号为 0，执行兼容保留的复制路径/文件列表功能。
3. 其他编号从命令表取出对应 `run` 和 `arg`。
4. 相对路径按 DLL 所在目录解析。
5. `arg` 中的占位符按当前选中文件替换。
6. 后台线程逐个处理选中的文件。
7. 每个文件调用一次 `CreateProcessW`。
...
占位符按文本直接替换，保持旧 C++ 版本的 `CString::Replace` 语义。路径如果可能包含空格，应在 `menu.xml` 中显式写引号。涉及 `cmd /c`、重定向、管道等 shell 语法时，仍应优先把复杂逻辑放进脚本或 exe 中。
...
- 第一个菜单项保留旧版复制路径/文件列表行为，避免破坏既有使用习惯。
